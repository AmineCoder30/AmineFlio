...
function Skills() {
  const skillsContainerRef = useAnimation('fadeUp');
  const skillsGridRef = useAnimation('stagger');
  ...
  return (
   ...
   <Title partOne="Things I’m" PartTwo="Great At" />
      <div ref={skillsContainerRef} className="mt-10 opacity-100">
        <div ref={skillsGridRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {technologies.map((tech, index) => (
            ...
            >
              ...
              <h3 className="text-text font-semibold text-center">{tech.name}</h3>
